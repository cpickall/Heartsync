...
ISR(TIMER2_COMPA_vect){                         // triggered when Timer2 counts to 124
  ...
  int N2 = sampleCounter - lastBeatTime2;
  ...
  //**********DO EVERYTHING AGAIN FOR THE SECOND SENSOR****************************
  
      //  find the peak and trough of the pulse wave
  if(Signal2 < thresh2 && N2 > (IBI/5)*3){       // avoid dichrotic noise by waiting 3/5 of last IBI
  ...
  if (N2 > 250){                                   // avoid high frequency noise
    if ( (Signal2 > thresh2) && (Pulse2 == false) && (N2 > (IBI/5)*3) ){        
      Pulse2 = true;                               // set the Pulse flag when we think there is a pulse
      IBI2 = sampleCounter - lastBeatTime2;         // measure time between beats in mS
      lastBeatTime2 = sampleCounter;               // keep track of time for next pulse
